...
* [ ] **Step 1.1: Repo Setup**
  * Initialize monorepo with two main directories: `/research_lab` (Python) and `/execution_engine` (Node.js/Python).
* [ ] **Step 1.2: Agent Orchestrator Initialization**
  * Deploy `agent-orchestrator` in the `/execution_engine`.
...
* [ ] **Step 2.1: Clone & Modify `autoresearch`**
  * Clone the repo into `/research_lab`.
* [ ] **Step 2.2: Build the Gym Environment**
  * Create `trading_env.py`. This script must load the CSV data from Phase 1. It gives the RL model three actions: `[BUY, SELL, HOLD]`.
  * Calculate reward based on Net Profit or Sharpe Ratio.
* [ ] **Step 2.3: Modify the `train.py` target**
  * Rewrite `autoresearch`'s target goal. Instead of minimizing validation loss, instruct the LLM to maximize the reward function from `trading_env.py`.
